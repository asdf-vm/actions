...
async function pluginTest() {
  ...
  const plugin = (core2.getInput("plugin", { required: false }) || // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
  process.env.GITHUB_REPOSITORY.split("/")[1]).replace("asdf-", "");
  const giturl = core2.getInput("giturl", { required: false }) || `https://github.com/${process.env.GITHUB_REPOSITORY}`;
  const gitref = (
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    core2.getInput("gitref", { required: false }) || process.env.GITHUB_SHA
  );
